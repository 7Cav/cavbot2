...
| Voice state events are checked against discordgo's state cache, which discordgo fills in gateway order before it starts any handler goroutine. The bot's own record of who is where can lag behind it, a stale voice state (`CONTEXT.md`). At handler entry an event whose channel differs from the cache's channel for that member is dropped and the record is left untouched; the member's latest event brings it current. Immediately before each action the handler reads the cache again and goes ahead only if its decision still holds: before the create and again before the move-into, the member is still in the hub, else no create, or the fresh channel is deleted through the guarded delete; before a delete, the cache shows nobody in the channel; before the hub refusal message, the member is still in the hub; before a handover, the successor is still in the channel, else the handover elects from occupants the cache confirms, else no owner. A guild missing from the cache counts as current at entry and as "cannot confirm" before an action, so the action is skipped; a missing guild never reads as an empty channel. A cached guild with no voice state for the member means the member is not connected, which agrees with a leave and supersedes a join or a move. The guarantee is that no handler acts on a decision the cache already shows superseded at that check. It is not full ordering: a newer event can land between the check and the API call, and no client-side mechanism closes that window. A dropped event and a skipped refusal message log at DEBUG. A skipped create, a skipped delete and an abandoned move with its compensating delete log at INFO. None reach Sentry. Rejected: `SyncEvents = true`, one line, but it serializes every handler, slash commands included, behind spawn calls and the warden purge sleeps, and its window is longer because the decision is made earlier; per-member gateway sequence gating, which covers a member's own reorders but not the cross-member case where a leave is handled before an earlier join and the delete goes out with someone inside; an in-order sequencer for the temp VC handlers, which covers everything but adds a buffer, a gap timeout and a per-session reset. The regression tests replay each scenario's handlers in reverse with the fake cache holding the final gateway state first, plus one test that moves the fake cache between a check and its action. | [#319](https://github.com/7Cav/cavbot2/issues/319), grilled 2026-09-20 |
