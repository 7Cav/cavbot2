...
- Moderator roles: the roles MEE6 lists as moderators for that hub, if any.
  ...
  the page. The pickers offer eligible roles only. If MEE6 lists a managed
  role, a bot's own role or the booster role, the picker leaves it out. A
  stored role that later loses eligibility shows as unavailable until a
  person unticks it (ADR 0012).
