...
- `BOT_DB_DSN` is the Postgres DSN for the bot's own store (`store/`), the hubs, spawned channels, guild settings and change log of the temporary voice channel feature. Unset: one WARN at startup (`BOT_DB_DSN not set, bot store disabled`), no store, every command works as before, and `/voice-rename` is registered but refuses every invocation (it stays registered so its Server Settings restriction, which Discord keys by command ID, survives a store-less start). Set: the bot pings it with a short retry, runs the embedded migrations, loads the hub rows and the guild-wide moderator roles into the temporary voice channel runtime (`Starting temp voice channels` with the count) and registers its gateway handlers, all before it opens the Discord session. With no hub rows nothing spawns; a hub row is what turns the feature on. A database it cannot reach or migrate stops the bot with `Bot store unavailable` and a non-zero exit, and `restart: unless-stopped` retries. The DSN is never logged, and a DSN that does not parse is reported without echoing it. `.env.example` leaves it empty so a copied `.env` stays inert; the compose value is in its comment.
- `PANEL_ADDR` gates the panel, the bot's web UI in `panel/`. Unset: one WARN at startup (`PANEL_ADDR not set, panel disabled`), no listener, every command works as before. Set (`:8080` under compose, reached over the external `edge` network and never published on the host): the bot reads the six variables below before the Discord session opens and stops with `Panel misconfigured` naming the first one missing. The panel also needs `BOT_DB_DSN`: its hub page reads the store on every load, so `PANEL_ADDR` set with `BOT_DB_DSN` empty means one more WARN (`BOT_DB_DSN not set, panel disabled`) and no listener. The listener starts after READY and logs `Panel listening` with the address, the base URL and the group allowlist.
- `PANEL_BASE_URL` is the public origin the panel is reached at (`https://cavbot2.7cav.us`); a trailing slash is stripped. Every absolute URL the panel emits comes from it; the panel reads no proxy header. The OAuth redirect URI is this plus `/auth/callback` and has to match a redirect URI registered on the forum client character for character. Required when `PANEL_ADDR` is set.
- `PANEL_OAUTH_CLIENT_ID` and `PANEL_OAUTH_CLIENT_SECRET` identify the confidential OAuth2 client registered in the forum's admin control panel. Required when `PANEL_ADDR` is set. The secret is never logged.
- `PANEL_OAUTH_AUTHORIZE_URL`, `PANEL_OAUTH_TOKEN_URL` and `PANEL_OAUTH_USERINFO_URL` are the forum's authorize, token and user endpoints (`https://7cav.us/oauth2/authorize`, `https://7cav.us/api/oauth2/token`, `https://7cav.us/api/me`). Required when `PANEL_ADDR` is set. The forum's "Back to the forum" link is derived from the authorize URL's origin. There is no revoke URL: sign-out ends the panel session and never calls the forum.
- `PANEL_GROUP_IDS` is the comma-separated forum group allowlist the group check reads on every request. Code default `71,47,44` (Genstaff, S6 HQ, Regimental Technical Aides); `.env.example` sets the same. A value that does not parse stops the bot with `Panel misconfigured`.
- `POSTGRES_PASSWORD` is read by the `postgres` service in `docker-compose.yml`, never by the bot. It sets the `cavbot` role's password on the first boot of an empty volume and is inert after that, so `BOT_DB_DSN` has to carry the same value the volume was created with. Empty makes the image refuse to start, and compose never starts the bot behind it. `.env.example` ships `change-me`.
