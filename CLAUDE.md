...
- `WARDEN_ROLE_BASE_NAME` — base name the `/warden` roles are composed from (`<base> Internal`, `<base> External`). Code default is `Verified Warden`; **`.env.example` sets the name the regiment is currently running** (`Verified Foxhole` since the Colonial change), so a copied `.env` never hits the code default — same arrangement as `LOA_NODE_IDS`. The roles get renamed between wars, so this is a per-deployment knob: a rename is a config edit plus a restart, not a release. Matching against Discord is **exact** — the configured base has to reproduce the role name character for character, and a wrong value fails every warden subcommand with `role not found`. The resolved value is logged at startup (`Warden role base name resolved`), which is the fastest way to confirm it reached the process.
