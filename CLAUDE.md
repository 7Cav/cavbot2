...
- `DISCORDGO_LOG_LEVEL` — discordgo's *own* gateway logging, separate from `LOG_LEVEL`. `ERROR` (default, and what the bot did before this existed) / `WARN` / `INFO` / `DEBUG`; unrecognised values mean `ERROR`. Set `WARN` when startup fails with `Discord session unavailable` — that is where discordgo names the frame the gateway sent instead of `READY`. `DEBUG` needs `LOG_LEVEL=DEBUG` too (the levels map faithfully, so both gates apply) and logs every unrecognised gateway event with its full payload — diagnostic only, never production.
