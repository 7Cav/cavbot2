...
- `WARDEN_ROLE_BASE_NAME` — base name the `/warden` roles are composed from (`<base> Internal`, `<base> External`). Default `Verified Warden`. The regiment renames these roles between wars, so this is a per-deployment knob: a rename is a config edit plus a restart, not a release. Matching against Discord is **exact** — the configured base has to reproduce the role name character for character, and a wrong value fails every warden subcommand with `role not found`. The resolved value is logged at startup (`Warden role base name`), which is the fastest way to confirm it reached the process.
