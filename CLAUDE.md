...
- `BOT_DB_DSN` is the Postgres DSN for the bot's own database (hub settings, spawned channel rows). Unset: one WARN at startup and the store is nil; every existing command works as before. Set: `store.Open` pings with retry, then the embedded migrations run under a session lock before the Discord session opens, and a failure panics so the restart policy retries (ADR 0012). The production host is the compose service `postgres`, on the `cavbot_internal` network. `POSTGRES_PASSWORD` in `.env` is read by compose for that service only, never by the bot.
- `PANEL_ADDR` is the listen address for the panel, cavbot2's web UI (`:8080` in production; `.env.example` leaves it empty). Unset: the panel does not listen and one INFO line says so. Set: `PANEL_BASE_URL`, `PANEL_OAUTH_CLIENT_ID`, `PANEL_OAUTH_CLIENT_SECRET`, `PANEL_OAUTH_AUTHORIZE_URL`, `PANEL_OAUTH_TOKEN_URL` and `PANEL_OAUTH_USERINFO_URL` are required, and a missing one panics in the configuration phase, before the store opens or Discord is touched. `PANEL_GROUP_IDS` is the forum group allowlist, default `71,47,44`. `PANEL_BASE_URL` plus `/auth/callback` must equal a redirect URI registered on the forum's OAuth client, character for character. Startup logs the address, base URL and allowlist, never the secret (ADR 0013).
...
`store.Store` is the persistence seam for the temporary voice channel feature: hubs and spawned channel rows today, guild settings and the change log with the tickets that need them. `store.Postgres` is production; `store.Fake` is the in-memory implementation other packages' tests bind. The package's own contract suite runs every test against both, and the Postgres half skips without `TEST_BOT_DB_DSN` (the README has the Docker one-liner). Migrations are goose SQL files under `store/migrations/`, embedded and applied at startup; every migration must stay compatible with the previous release (ADR 0012).

### Panel (panel/)

The panel is an HTTP server inside the bot process, started after READY. `panel.ConfigFromEnv` reads the `PANEL_` variables, `panel.New` builds the server, `Server.Handler()` is the mux behind `net/http`'s cross-origin protection, and `Server.Run` prunes expired sessions. Sign-in is Xenforo OAuth2 with PKCE; sessions and pending sign-ins live in memory for two hours and five minutes. `requireSession` runs the group check against the forum on every request and ends the session with cause `expired` or `no-group`, which the sign-in page renders under a `data-cause` attribute. Tests drive the handler with `httptest` and a stub forum; there is no seam. Handlers on later pages call a service layer, never the store or Discord directly (ADR 0013).
...
go mod tidy
docker start cavbot2-test-pg >/dev/null 2>&1 || docker run -d --name cavbot2-test-pg -e POSTGRES_USER=cavbot -e POSTGRES_PASSWORD=cavbot -e POSTGRES_DB=cavbot_test -p 5432:5432 postgres:18-alpine >/dev/null
until docker exec cavbot2-test-pg pg_isready -U cavbot -d cavbot_test >/dev/null 2>&1; do sleep 1; done
export TEST_BOT_DB_DSN='postgres://cavbot:cavbot@localhost:5432/cavbot_test?sslmode=disable'
go test ./... -race -cover -covermode=atomic | tee /tmp/cover.log
./.github/scripts/check-coverage-floors.sh < /tmp/cover.log
go build -o cavbot2
```

Mirrors `.github/workflows/build_test.yml` exactly: the two `docker` lines stand in for the workflow's Postgres service container, which the store package's tests need to reach their coverage floor (ADR 0012). `pipefail` ensures a `go test` failure isn't swallowed by `tee`.
