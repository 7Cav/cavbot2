...
- `BOT_DB_DSN` — Postgres DSN for the bot's own database (hub settings, spawned channel rows). Unset: one WARN at startup and the store is nil; every existing command works as before. Set: `store.Open` pings with retry, then the embedded migrations run under a session lock before the Discord session opens, and a failure panics so the restart policy retries (ADR 0012). The production host is the compose service `postgres`, on the `cavbot_internal` network. `POSTGRES_PASSWORD` in `.env` is read by compose for that service only, never by the bot.
- `PANEL_ADDR` — listen address for the panel, cavbot2's web UI (`:8080`). Unset: the panel does not listen and one INFO line says so. Set: `PANEL_BASE_URL`, `PANEL_OAUTH_CLIENT_ID`, `PANEL_OAUTH_CLIENT_SECRET`, `PANEL_OAUTH_AUTHORIZE_URL`, `PANEL_OAUTH_TOKEN_URL` and `PANEL_OAUTH_USERINFO_URL` are required and a missing one panics at startup. `PANEL_GROUP_IDS` is the forum group allowlist, default `71,47,44`. `PANEL_BASE_URL` plus `/auth/callback` must equal a redirect URI registered on the forum's OAuth client, character for character. Startup logs the address, base URL and allowlist, never the secret (ADR 0013).
