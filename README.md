...
| `PANEL_ADDR` | The panel, the bot's web UI, does not listen; the bot logs `PANEL_ADDR not set, panel disabled` once. Set it (`:8080` under compose) and every other `PANEL_*` variable but `PANEL_GROUP_IDS` is required, and so is `BOT_DB_DSN`, because the hub page reads the store on every load; a missing one stops the bot with `Panel misconfigured` before the Discord session opens. The listener starts after READY and logs `Panel listening`. `.env.example` documents each `PANEL_*` variable. |
